...
// Formula from lecture/slides:
  // SYSCLK = ((fSRC / ((Q+1)(N+1))) * MINT) / (PSYSDIV + 1)
  //
  // Using:
  // fSRC = 25 MHz (MOSC)
  // Q = 0
  ...
  //
  // Therefore:
  // SYSCLK = ((25 / ((0+1)(4+1))) * 22) / (4+1)
  //        = ((25 / 5) * 22) / 5
  //        = (5 * 22) / 5
  //        = 22 MHz
	
	
	//    fVCO = (fXTAL/(Q + 1)/(N + 1))*(MINT + (MFRAC/1,024))
//    For this project, the PLL is configured to produce a 22 MHz system clock.
//    Using MOSC = 25 MHz and choosing:
//    Q = 0, N = 4, MINT = 22, MFRAC = 0, PSYSDIV = 4
//
//    First calculate the PLL output:
//    fPLL = (25,000,000 / (0 + 1) / (4 + 1)) * (22 + (0 / 1,024))
//         = (25,000,000 / 5) * 22
//         = 5,000,000 * 22
//         = 110,000,000 Hz
//
//    Then divide by (PSYSDIV + 1):
//    SYSCLK = 110,000,000 / (4 + 1)
//           = 22,000,000 Hz = 22 MHz
	
	
	
	#define FXTAL 25000000  // 25 MHz crystal on the board
#define Q            0
#define N            4  // chosen so reference frequency stays in valid range
#define MINT        22
#define MFRAC        0  // zero to reduce jitter
  //    SysClk = fVCO / (PSYSDIV + 1)
#define SYSCLK (PSYSDIV+1)
